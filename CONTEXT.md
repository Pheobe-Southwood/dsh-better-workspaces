...
**草稿交接（draft handoff）**：
择基创建从源会话切到目标会话时对草稿的条件转移：先记住源草稿、附件顺序及修订，验证源未被继续改写后把文本与附件写入目标、重绑文件上传并读回确认，随后才清空源并打开目标；任一步失败都保留或恢复源草稿。公开接口不能重建结构化引用 chip，因此检测到 chip 时失败关闭、绝不把它悄悄扁平化成文本。
...
一次择基创建的稳定身份。相同请求重试只会指向同一个托管 worktree 及其唯一工作区；同一身份若被拿来表示另一项请求则冲突失败。收到成功答复意味着这些实体都已可按收据找回，即使先前答复在途中丢失也不靠删除补偿。
