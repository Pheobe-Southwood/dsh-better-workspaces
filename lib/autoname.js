...
/** Naming output budget — a branch slug never needs more. */
const LLM_MAX_TOKENS = 64;
...
const SYSTEM_PROMPT = [
  'Generate one git branch name for a coding-agent worktree from the supplied user prompt.',
  ...
  'The branch must be a valid git ref: lowercase letters, numbers, hyphens, and slashes only,',
  ...
  'The prompt may be in any language; the branch name is always English.',
  'Reply with the branch name only — no quotes, no explanation.',
].join('\n');
...
export function createAutoNamer(ctx, hub) {
  ...
  async function attempt(session, text) {
    ...
    try {
      ...
      if (!generated) return;
      const cleaned = cleanBranchName(generated);
      const verdict = validateBranchSlug(cleaned);
      if (!verdict.valid || cleaned === placeholder) return;
