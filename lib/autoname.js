 ...
 *
 * Modified port of Paseo's WorkspaceAutoName.attemptFirstAgentBranchAutoName
 * (+ worktree-branch-name-generator prompt contract, branch-slug validation,
 * findAvailableBranchName suffixing). The Apache-2.0 source revision and full
 * attribution are recorded in NOTICE; this file is substantially modified for
 * Cordis/DSH lifecycle, authorization, failure and transaction semantics:
 *
...
import { isDirfdPinSupported, procFdPath, verifyStatIdentity } from './stable.js';
...
export function createAutoNamer(ctx, hub, options = {}) {
  ...
  const withAuthorizedRoot = async (capability, task) => {
    ...
    if (isDirfdPinSupported()) {
      ...
      try {
        ...
        return await task(procFdPath(directory));
      } finally {
    ...
    // identity is the proof; Git discovers from the working tree (ADR 0013).
    if (!(await verifyStatIdentity(current.cwd, current.identity))) return null;
    if (current.mainRepoRoot && current.mainRepoRoot !== current.cwd
      && !(await verifyStatIdentity(current.mainRepoRoot, current.mainIdentity))) return null;
    return await task(current.cwd);
  };
  ...
  async function generateBranchName(promptText, sessionId) {
    ...
    try {
      ...
      /* a missing optional route leaves auto-naming disabled */
    }
            ...
            try {
              ...
              if (rolledBack.ok) {
                ...
                if (restored === false
                  ...
                  throw new Error(`${String(metadataError?.message ?? metadataError)}; branch compensation completed but metadata restoration failed`, { cause: metadataError });
                }
              }
              throw new Error(
