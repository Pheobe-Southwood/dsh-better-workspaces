...
import { isDirfdPinSupported, normalizeGitPath } from './stable.js';
...
export function runGit(args, opts = {}) {
  ...
  return limit(
    () => pinnedGitEnvironment.run(callerGitEnvironment, () => scopedGitAbortSignal.run(callerAbortSignal, async () => {
      ...
      try {
        if (isDirfdPinSupported() && typeof cwd === 'string') {
...
export async function detectRepo(path) {
  ...
  // Git prints forward slashes on Windows (`C:/Users/…`); normalize so the
  // outputs compare with realpath-derived canonical paths (ADR 0013).
  const repoRoot = normalizeGitPath(top.stdout.trim());
  ...
  const gitCommonDir = common.ok ? normalizeGitPath(common.stdout.trim()) : null;
  const gitDir = own.ok ? normalizeGitPath(own.stdout.trim()) : null;
...
export async function listWorktreesRaw(repoRoot) {
  ...
  for (const line of r.stdout.split('\n')) {
    ...
    if (line.startsWith('worktree ')) {
      current = { path: normalizeGitPath(line.slice('worktree '.length)), branch: null, head: null, detached: false, bare: false, prunable: false };
      continue;
    }
    if (!current) continue;
