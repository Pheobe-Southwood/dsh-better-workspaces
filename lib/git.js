...
function gitEnvironment(overrides) {
  ...
  return { ...result, ...(overrides || {}), ...GIT_ENV };
}
...
async function inspectExecutableLocalConfig(cwd, args, timeout, env) {
  const patterns = executableConfigPatterns(args);
  if (!cwd || patterns.length === 0) return { ok: true };
  const result = await new Promise((resolveResult) => {
    execFile(
      'git',
      [...SAFE_GIT_CONFIG, 'config', '--includes', '--show-scope', '--name-only', '-z', '--get-regexp', '.'],
      { cwd, timeout: Math.min(timeout, 10000), maxBuffer: 1024 * 1024, env: gitEnvironment(env) },
      (error, stdout, stderr) => {
  ...
  return limit(
    async () => {
      ...
      try {
        ...
        const guard = await inspectExecutableLocalConfig(executionCwd, args, timeout, env);
        if (!guard.ok) {
