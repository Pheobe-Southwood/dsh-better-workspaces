 ...
 */
import { EMPTY_TREE, runGit, headState, tryMergeBase, resolveBestComparisonBaseRef, resolveDefaultBranch, hasRemoteBranch } from './git.js';
...
export async function commitDiff(cwd, sha, opts = {}) {
  ...
  if (!parents.ok) throw new Error((parents.stderr || 'cannot resolve commit').trim().slice(0, 600));
  const parts = (parents.stdout.trim().split(/\s+/) || []).filter(Boolean);
  if (parts.length === 0) throw new Error('cannot resolve commit');
