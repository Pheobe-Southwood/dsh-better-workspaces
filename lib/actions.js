...
import { archiveWorktree, mainRepoRootOf, readMetadata, validateManagedWorktree } from './worktree.js';
...
function validPullProvenance(metadata) {
  return Boolean(metadata?.intent === 'pr-checkout'
    ...
    && FULL_OID.test(metadata.prHeadSha || '')
    && forgeRepositoryKey({ host: metadata.pullHost, owner: metadata.pullOwner, repo: metadata.pullRepo }));
}

async function invalidatePushedRepository(cwd) {
  const identity = await forgeIdentityForCwd(cwd).catch(() => null);
  const key = forgeRepositoryKey(identity);
  if (!key) return;
  invalidatePr(`${key}@`);
  invalidateForgeList(identity);
}
...
async function contextOf(cwd) {
  ...
  const pullIdentity = pullProvenanceValid
    ...
    : null;
  let pullIdentityTrusted = false;
  if (pullIdentity) {
    const managed = await validateManagedWorktree(cwd).catch(() => ({ ok: false }));
    const upstreamIdentity = parseGithubRemote(await remoteUrl(cwd, 'upstream').catch(() => null));
    const pullKey = forgeRepositoryKey(pullIdentity);
    pullIdentityTrusted = managed.ok && Boolean(pullKey) && [originIdentity, upstreamIdentity]
      .some((identity) => forgeRepositoryKey(identity) === pullKey);
  }
  const github = pullIdentityTrusted ? pullIdentity : originIdentity;
  return {
    branchInfo, metadata, defaultBranch, baseName, bestBase, upstream, remote, github,
    pullIdentityTrusted, pullProvenanceValid, pullProvenanceMalformed,
  };
}
...
export async function pushAction(cwd) {
  const context = await contextOf(cwd);
  const { branchInfo, metadata, pullIdentityTrusted, pullProvenanceValid, pullProvenanceMalformed, upstream } = context;
  ...
  let args;
  if (metadata?.intent === 'pr-checkout') {
    const expectedUpstream = pullProvenanceValid ? `refs/remotes/origin/${metadata.pullHeadRef}` : null;
    if (pullProvenanceMalformed || !pullIdentityTrusted || upstream.upstreamRef !== expectedUpstream) {
      return { ok: false, reasonKey: 'actions.push.prCheckout' };
    }
    // The local checkout may be uniquified (head-1) while safely tracking the
    // verified origin/head. An explicit refspec avoids push.default=simple
    // rejecting that intentional name mismatch.
    args = ['push', 'origin', `${localRef}:refs/heads/${metadata.pullHeadRef}`];
  } else {
    args = upstream.upstreamRef ? ['push'] : ['push', '-u', 'origin', `${localRef}:${localRef}`];
  }
  const r = await runGit(args, { cwd, timeout: 180000 });
  if (!r.ok) return { ok: false, message: (r.stderr || r.stdout).trim().slice(0, 600) };
  await invalidatePushedRepository(cwd);
  return { ok: true };
}
...
async function withTargetDirfd(target, task) {
  if (isDirfdPinSupported()) {
    ...
    try {
      ...
      const gitDir = target.gitDir || join(target.cwd, '.git');
      const commonDir = target.gitCommonDir || gitDir;
      const openGit = async (path, expected, label) => {
      ...
      gitHandle = await openGit(gitDir, target.gitDirIdentity, 'Git directory');
      commonHandle = commonDir === gitDir ? gitHandle : await openGit(commonDir, target.gitCommonIdentity, 'Git common directory');
      return await withPinnedGitEnvironment({
        GIT_DIR: procFdPath(gitHandle),
        GIT_COMMON_DIR: procFdPath(commonHandle),
        GIT_WORK_TREE: stableCwd,
      }, () => task(stableCwd));
    } catch (error) {
  ...
  // Path mode (paseo model): prove the same identities by stat, then run the
  // merge against canonical paths with ordinary Git discovery (ADR 0013).
  try {
    if (!(await verifyStatIdentity(target.cwd, target.identity))) return { ok: false, status: 409, reason: 'stale-target' };
    if (target.mainRepoRoot && target.mainRepoRoot !== target.cwd
      && !(await verifyStatIdentity(target.mainRepoRoot, target.mainIdentity))) {
      return { ok: false, status: 409, reason: 'stale-target' };
    }
    const gitDir = target.gitDir || join(target.cwd, '.git');
    const commonDir = target.gitCommonDir || gitDir;
    if (!(await verifyStatIdentityFields(gitDir, target.gitDirIdentity?.dev, target.gitDirIdentity?.ino))) {
      return { ok: false, status: 409, reason: 'stale-target' };
    }
    if (commonDir !== gitDir
      && !(await verifyStatIdentityFields(commonDir, target.gitCommonIdentity?.dev, target.gitCommonIdentity?.ino))) {
      return { ok: false, status: 409, reason: 'stale-target' };
    }
    return await task(target.cwd);
  } catch (error) {
    return { ok: false, status: 409, reason: 'stale-target', message: String(error?.message ?? error) };
  }
}
...
export async function updateFromBaseAction(cwd) {
  const { branchInfo, bestBase } = await contextOf(cwd);
  if (!branchInfo.ok) return inspectionFailure('head', branchInfo);
  if (!branchInfo.branch) return { ok: false, reasonKey: 'actions.merge.detached' };
...
export async function createPrAction(cwd, { title, body, draft = false } = {}) {
  const { branchInfo, metadata, baseName, github } = await contextOf(cwd);
  if (!branchInfo.ok) return inspectionFailure('head', branchInfo);
  // A checked-out PR already has forge identity and head semantics. Never
  // publish its local (possibly owner-prefixed or uniquified) branch to origin
  // as the preliminary step for creating a second PR.
  if (metadata?.intent === 'pr-checkout') return { ok: false, reasonKey: 'actions.pr.checkout' };
  ...
  const localRef = `refs/heads/${branchInfo.branch}`;
  const push = await runGit(
    ['push', '-u', 'origin', `${localRef}:${localRef}`],
    { cwd, timeout: 180000 },
  );
  ...
  invalidatePr(`${forgeRepositoryKey(github)}@`);
  // Every linked worktree shares this canonical repository page, so one
  // repository-scoped invalidation refreshes all of them.
  invalidateForgeList(github);
  return { ok: true, url: r.url };
}

export async function mergePrAction(cwd, { method = 'squash', auto = false } = {}) {
  const { branchInfo, metadata, github, pullIdentityTrusted, pullProvenanceValid, pullProvenanceMalformed } = await contextOf(cwd);
  if (!github) return { ok: false, reasonKey: 'actions.pr.notGithub' };
  if (!branchInfo.branch) return { ok: false, reasonKey: 'actions.push.noBranch' };
  if (pullProvenanceMalformed || (pullProvenanceValid && !pullIdentityTrusted)) {
    return { ok: false, reasonKey: 'actions.mergePr.noPr' };
  }
  const prHeadRef = pullIdentityTrusted ? metadata.pullHeadRef : branchInfo.branch;
  const pullNumber = pullIdentityTrusted ? metadata.pullNumber : undefined;
  ...
  if (!pr || (Number.isSafeInteger(pullNumber) && pr.number !== pullNumber)) {
    return { ok: false, reasonKey: 'actions.mergePr.noPr' };
  }
  if (pr.state !== 'open') return { ok: false, reasonKey: 'actions.mergePr.notOpen' };
  const r = await mergePullRequest({ host: github.host, owner: github.owner, repo: github.repo, number: pr.number, method, auto });
  if (!r.ok) return { ok: false, message: r.message };
  invalidatePr(`${forgeRepositoryKey(github)}@`);
  invalidateForgeList(github);
  return { ok: true };
...
export async function autoMergeOffAction(cwd) {
  ...
  const r = await disableAutoMerge({ host: github.host, owner: github.owner, repo: github.repo, number: pr.number });
  if (!r.ok) return { ok: false, message: r.message };
  invalidatePr(`${forgeRepositoryKey(github)}@`);
  invalidateForgeList(github);
  return { ok: true };
...
export async function archiveAction(path, { force = false, beforeRemove = null } = {}) {
  return archiveWorktree(path, { force, beforeRemove });
}
...
export function buildActionLadder(snapshot, { agentRunning = false } = {}) {
  ...
  const prPushBlocked = Boolean(s.prPollingDisabled || (s.prHeadRef
    && s.upstream?.ref !== `refs/remotes/origin/${s.prHeadRef}`));
  const unpushed = prPushBlocked
    ? null
    : s.upstream
      ? s.upstream.ahead
      : originKnown && s.originDelta
        ? s.originDelta.ahead
        : originKnown && aheadKnown && s.aheadBehind
          ? s.aheadBehind.ahead
          : null;
  ...
  } else if (s.github && !s.prHeadRef && !s.prPollingDisabled && branch && aheadKnown && (s.aheadBehind?.ahead ?? 0) > 0) {
    entries.push(
      guard({
        id: 'createPr',
        disabled: false,
...
export async function executeAction(hub, cwd, actionName, params = {}, controls = {}) {
  ...
  switch (actionName) {
    ...
    case 'autoMergeOff':
      ...
      break;
    case 'archive':
      result = await archiveAction(cwd, { ...params, beforeRemove: controls.beforeArchiveRemove || null });
      break;
    default:
