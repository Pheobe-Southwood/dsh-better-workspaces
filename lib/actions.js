 ...
 */
export async function mergeToBaseAction(cwd, { authorizeTarget } = {}) {
  ...
  const ownerIsMain = (await safeRealpath(owner.path)) === (await safeRealpath(mainRoot));
  if (!ownerIsMain && (!authorizeTarget || !(await authorizeTarget(owner.path)))) {
    return {
      ok: false,
      status: 403,
      reason: 'target-unauthorized',
      reasonKey: 'actions.mergeToBase.targetUnauthorized',
      message: 'base worktree is outside the authorized Workspace boundary',
    };
  }
  ...
  const r = await runGit(['merge', '--no-edit', `refs/heads/${branchInfo.branch}`], { cwd: owner.path, timeout: 120000 });
  if (!r.ok) {
...
export async function executeAction(hub, cwd, actionName, params = {}, controls = {}) {
  ...
  switch (actionName) {
    ...
    case 'discard':
      ...
      break;
    case 'mergeToBase':
      result = await mergeToBaseAction(cwd, { authorizeTarget: controls.authorizeTarget });
      break;
    case 'updateFromBase':
