...
export async function createPrAction(cwd, { title, body, draft = false } = {}) {
  const { branchInfo, baseName, bestBase, github, pullIdentityTrusted, pullProvenanceValid, pullProvenanceMalformed } = await contextOf(cwd);
  if (!branchInfo.ok) return inspectionFailure('head', branchInfo);
...
export function buildActionLadder(snapshot, { agentRunning = false } = {}) {
  ...
  } else if (s.github && !s.prPollingDisabled && branch && aheadKnown && (s.aheadBehind?.ahead ?? 0) > 0) {
    entries.push(
      guard({
        id: 'createPr',
        disabled: false,
