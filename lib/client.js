...
`;
		...
		function HeroControl() {
			...
			react.useEffect(() => {
				...
				const healed = !summaryTitle && ws0 && prefix && det.branch && ws0.title === basenameOf(cwd)
					? prefix + " · " + det.branch
					: null;
		 ...
		 *
		 * Deliberately WITHOUT a `guide` entry. The right Sidebar's `defaultSeed`
		 * sends a session's first open straight to a type only while exactly ONE
		 * guide entry exists — the official files panel holds that position today.
		 * A second entry would turn every session's first sidebar open into the
		 * guide chooser, i.e. this plugin would silently change dsh's default. The
		 * composer's pill stays the door instead.
