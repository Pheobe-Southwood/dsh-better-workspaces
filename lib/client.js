...
`;
		...
		const zh = {
			...
			"hero.modeWorktreePick": "新建 worktree（选基分支）…",
		...
		const en = {
			...
			"hero.modeWorktreePick": "New worktree (pick base)…",
			"hero.wtTrigger": "wt: {branch}",
		...
		function HeroControl() {
			...
			async function createWorktree() {
				...
				setError(null);
				try {
					// explicit-name path: branch-off with the user's chosen name (never auto-renamed later)
					const body = {
						cwd,
						base: branchPick || (branches && branches.defaultBranch) || undefined,
						intent: "branch-off",
					...
					children: (close) =>
						h(MenuList, {
							items: [
								...
								{ id: "new", label: t("hero.modeWorktree") },
								{ id: "pick", label: t("hero.modeWorktreePick"), active: staging },
							],
							onSelect: (id) => {
								close();
								if (id === "local") goLocal();
								else if (id === "new") createWorktreeWith(null);
								else openBranchList();
							},
						}),
				}),
				staging && h(Popover, {
					...
					trigger: ({ open, toggle }) =>
						h("button", { type: "button", className: "dsh-bw-hero-btn", onClick: toggle, disabled: Boolean(busy), "aria-expanded": open },
							h(BranchIcon),
							h("span", { className: "dsh-bw-hero-label" }, t("hero.pickBase", { branch: effectiveBranch })),
							h("span", { className: "dsh-bw-hero-chevron" }, "▾")),
					children: (close) =>
						branches === null
			...
			async function createWorktreeWith(baseBranch) {
				setBusy(t("hero.creating"));
				setError(null);
				try {
					// paseo semantics: the picked branch is the BASE; a new placeholder
					// branch is always cut (slug seeds both directory and — on hosts new
					// enough — the placeholder branch name, later LLM-renamed)
					const body = { cwd, intent: "branch-off", slug: mnemonicSlug() };
					if (baseBranch) body.base = baseBranch;
					const result = await apiPost("/worktrees", body);
					if (!result.ok) throw new Error(result.message || "worktree create failed");
					setBusy(t("hero.switching"));
					await openWorkspaceFor(result.path);
