...
`;
		...
		async function openWorkspaceFor(path) {
			...
			const sessionId = typeof raw === "string" ? raw : raw && raw.value ? raw.value.sessionId || raw.value.session || raw.value : raw;
		...
		function HeroControl() {
			...
			async function prepareStagedWorktree(baseBranch, explicit) {
				...
				try {
					...
					const raw = await appCtx.sessions.create({ workspaceId });
					targetId = typeof raw === "string" ? raw : raw && raw.value ? raw.value.sessionId || raw.value.session || raw.value : raw;
					...
					try {
						...
						const renamed = await appCtx.workspaces.rename({ workspaceId, title });
						if (renamed && renamed.ok === false && renamed.error && renamed.error.code === "workspace/name-conflict") {
							await appCtx.workspaces.rename({ workspaceId, title: title + "-2" });
						}
					} catch { /* cosmetic */ }
					setStaging(false);
