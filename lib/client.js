...
`;
		...
		const zh = {
			...
			"hero.stageHint": "选定基分支或点「立即创建」后跳转新会话",
		...
		const en = {
			...
			"hero.stageHint": "creates and jumps as soon as you pick a base",
		...
		function HeroControl() {
			...
			react.useEffect(() => {
				...
				(async () => {
					try {
						...
						if (!ws || ws.title === summaryTitle) return;
						const renamed = await appCtx.workspaces.rename({ workspaceId: ws.workspaceId, title: summaryTitle });
						if (!renamed.ok) titleSynced.current = false; // retry on next title change
					} catch {
			...
			/* Create-on-arm (ADR 0004 amendment 2): an explicit base pick, an
			 * explicit name confirmation, or the Create-now button creates the
			 * worktree immediately and jumps — browsing stages nothing */
			async function createWorktreeWith(baseBranch, explicit) {
				...
				try {
					...
					else body.base = baseBranch || branchPick || (branches && branches.defaultBranch) || undefined;
					...
					if (opened.workspace) {
						const repoBase = basenameOf(detect.mainRepoRoot || detect.repoRoot || cwd);
						const title = (repoBase ? repoBase + " · " : "") + (result.branch || "");
						try {
							let renamed = await appCtx.workspaces.rename({ workspaceId: opened.workspace.workspaceId, title });
							if (!renamed.ok && renamed.error && renamed.error.code === "workspace/name-conflict") {
								await appCtx.workspaces.rename({ workspaceId: opened.workspace.workspaceId, title: title + "-2" });
							}
						} catch { /* cosmetic */ }
					}
					setStaging(false);
			...
			return h(react.Fragment, null,
				...
				staging ? h("button", {
					...
					onClick: () => createWorktreeWith(null),
		...
		exports.__bwTest = { failureMessage, mnemonicSlug, basenameOf };
