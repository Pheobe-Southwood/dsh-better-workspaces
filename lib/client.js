...
`;
		...
		async function renameWorkspaceTolerant(workspaceId, title) {
			try {
				...
				if (renamed && renamed.ok === false && renamed.error && renamed.error.code === "workspace/name-conflict") {
					const retry = await appCtx.workspaces.rename({ workspaceId, title: title + "-2" });
					return !(retry && retry.ok === false);
				}
				return !(renamed && renamed.ok === false);
			} catch {
				return false;
			}
		}
		...
		function createSidebarInjector(ctx) {
			...
			async function refreshManagedTitles() {
				...
				} catch { /* older host without the route → no icons */ }
			}
			...
			react.useEffect(() => {
				...
				(async () => {
					try {
						...
						const title = prefix ? prefix + " · " + summaryTitle : summaryTitle;
						if (ws.title === title) return;
		...
		exports.__bwTest = { failureMessage, mnemonicSlug, basenameOf, getFileIconSvg, desaturateHexColor };
