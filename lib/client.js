...
`;
		...
		function resolveForgeSessionInput(sessions, sessionId) {
			try {
				const conversation = appCtx && appCtx.get ? appCtx.get("uiConversation") : null;
		 ...
		 * editor's own caret span, degrading to a plain-text append when the chip
		 * path is unavailable.
		 *
		 * The span must come from the shell's `caretSpan()` (with no selection it
		 * answers `detectText.length`, the draft end) — never rebuilt by hand from
		 * `InputState.occurrences`: an occurrence's `length` is measured in
		 * CLIPBOARD coordinates while a chip occupies ZERO characters of the
		 * detect projection, so `offset + length` overshoots the draft end on the
		 * first chip. `caretSpan()` also yields the LIVE `draftRev` the shell's
		 * span CAS compares against.
		 * @param args - item, the sessions service, the resolved facade, and the
		 *   fallback span/draft for a shell that predates `caretSpan`.
		...
		function attachForgeReference(args) {
			...
			try {
				if (target && typeof target.insertReference === "function") {
					const span = typeof target.caretSpan === "function" ? target.caretSpan() : args.fallbackSpan;
					done = target.insertReference(reference, {
						start: span.start,
						end: span.end,
						draftRev: typeof span.draftRev === "number" ? span.draftRev : args.fallbackSpan.draftRev,
					}) === true;
				}
			} catch {
				done = false;
			...
			if (!done && target && typeof target.setDraft === "function") {
				// Degradation: append the plain token at the draft end. It still
				// expands through the codec on send; only the chip node is absent.
				try {
					const current = target.snapshot && typeof target.snapshot.draft === "string"
						? target.snapshot.draft
						: args.fallbackDraft;
					target.setDraft(current === "" ? text : current.replace(/\s+$/, "") + " " + text);
		...
		function ForgeAttachControl(props) {
			...
			function insert(item) {
				...
				attachForgeReference({
					item,
					sessions,
					sessionInput: resolveForgeSessionInput(sessions, sessionId),
					// Fallback for a shell without caretSpan(); the live one wins.
					fallbackSpan: { start: draftEnd(), end: draftEnd(), draftRev },
					fallbackDraft: draft,
				});
				setOpen(false);
			}
