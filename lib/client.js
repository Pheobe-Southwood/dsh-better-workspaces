...
`;
		...
		function createGitFeed() {
			...
			function merge(snapshot) {
				if (disposed || !snapshot || typeof snapshot.cwd !== "string") return;
			...
			return {
				...
				untrack(cwd) {
					...
					store.set((s) => {
						...
						if (count === 1) delete tracked[cwd];
						else tracked[cwd] = count - 1;
						return { ...s, tracked };
					});
				},
				refresh(cwd) {
		...
		async function detectPath(path, signal) {
			...
			if (!(signal && signal.aborted)) detectCache.set(path, { at: Date.now(), value });
			return value;
		}
		...
		async function sessionTouchedPaths(sessionId, cwd, force) {
			...
			if (touchedSeq.get(key) === seq) touchedCache.set(key, { at: Date.now(), paths });
			return paths;
		}
		...
		function queueWorkspaceRename(workspaceId, title) {
			let state = workspaceRenameQueues.get(workspaceId);
			if (!state) {
				state = { desired: title, promise: null };
				workspaceRenameQueues.set(workspaceId, state);
			...
			if (!state.promise) {
				state.promise = (async () => {
					...
					for (;;) {
						...
						outcome = await renameWorkspaceTolerant(workspaceId, target);
						if (state.desired === target) return outcome;
					}
				})().finally(() => {
					if (workspaceRenameQueues.get(workspaceId) === state) workspaceRenameQueues.delete(workspaceId);
				});
			}
		...
		const workspaceMutationLeases = new Set();
		function acquireWorkspaceMutation(cwd) {
			const key = String(cwd || "");
			if (!key || workspaceMutationLeases.has(key)) return null;
			workspaceMutationLeases.add(key);
			return () => workspaceMutationLeases.delete(key);
		}
		...
		async function openWorkspaceFor(path, expectedSessionId) {
			...
			const flight = (async () => {
				const currentStillOwned = () => !expectedSessionId || currentSessionId() === expectedSessionId;
				...
				if (!currentStillOwned()) {
					try { await appCtx.workspaces.archiveSession(sessionId); } catch { /* exact unused target */ }
					throw ownershipError();
				}
				appCtx.sessions.open(sessionId);
		...
		function HeroControl() {
			...
			useEffect(() => {
				...
				detectPath(cwd, token.controller.signal)
					.then((value) => { if (requestOwner.current.owns(token)) setDetect({ cwd, value }); })
					.catch(() => {})
		...
		function forgeInsertSpan(target, fallbackInput) {
			const liveInput = target && target.snapshot ? target.snapshot : null;
			const input = liveInput || fallbackInput;
			const detectEnd = referenceDetectEnd(input);
			const rev = target && Number.isSafeInteger(target.rev)
				? target.rev
				: input && Number.isSafeInteger(input.draftRev) ? input.draftRev : null;
			...
			const atEnd = Boolean(live)
				&& Number.isSafeInteger(live.start)
				&& Number.isSafeInteger(live.end)
				&& live.start === live.end
				&& live.end === detectEnd;
			forgeTrace("caret", {
				start: live && Number.isSafeInteger(live.start) ? live.start : null,
				end: live && Number.isSafeInteger(live.end) ? live.end : null,
				shellRev: rev, detectEnd, atEnd,
			});
			const at = atEnd ? live.end : detectEnd;
			return { span: { start: at, end: at, draftRev: rev }, source: atEnd ? "caret" : "draft-end" };
		}
