...
`;
		...
		function ForgeAttachControl(props) {
			...
			/**
			 * Detect-coordinate end of the draft. Chips occupy placeholder text in
			 * that projection while `draft` carries their clipboard form, so the
			 * tail sits one trailing segment past the last chip — exactly what the
			 * shell's own caretSpan() answers with no live selection. An
			 * under-estimate is safe: insertReference refuses on a failed CAS and
			 * the caller degrades to a plain-text append.
			 */
			function draftEnd() {
				let end = 0;
				for (const occurrence of occurrences) {
					const offset = Number(occurrence && occurrence.offset);
					const length = Number(occurrence && occurrence.length);
					if (!Number.isFinite(offset)) continue;
					end = Math.max(end, offset + (Number.isFinite(length) ? length : 0));
				}
				if (occurrences.length === 0) return draft.length;
				const tail = draft.length - occurrences.reduce((sum, occurrence) => sum + (Number(occurrence && occurrence.clipboardText ? occurrence.clipboardText.length : 0) || 0), 0);
				return end + Math.max(tail, 0);
			}
			...
			function insert(item) {
				...
				const reference = {
					...
					// and an issue/PR is none of them — the chip keeps its "@" marker.
				...
				try {
					const conversation = appCtx.get("uiConversation");
					const binding = sessions.binding ? sessions.binding(sessionId) : null;
					sessionInput = conversation && conversation.input && binding
						? conversation.input.for(binding.ctx || binding)
						: null;
				} catch {
					sessionInput = null;
				}
				let done = false;
				try {
					// Append at the draft end: the picker owns no editor selection
					// (the read-only caret live in InputBar), so an empty span at
					// the tail is the only insertion point we can name — the
					// revision CAS is what actually validates it.
					if (sessionInput && typeof sessionInput.insertReference === "function") {
						const at = draftEnd();
						done = sessionInput.insertReference(reference, {
							start: at, end: at, draftRev,
						}) === true;
					}
				} catch {
					done = false;
				}
				if (!done && sessionInput && typeof sessionInput.setDraft === "function") {
					// Degradation: the plain token still becomes a chip (the text-ref
					// decoration scans the lexicon) and still expands through the
					// codec — only the caret lands at the end of the draft.
					try {
						sessionInput.setDraft(draft === "" ? text : draft.replace(/\s+$/, "") + " " + text);
					} catch { /* composer refused the write: nothing to do */ }
				}
				setOpen(false);
			}
		...
		exports.__bwTest = { failureMessage, mnemonicSlug, basenameOf, collectTouched, normalizeTouchedPaths, heroModeItems, liveDetect, pillLabel, SIDEBAR_DIFF_ID, SIDEBAR_DIFF_KIND, ForgeAttachControl, ForgePicker, ForgeRow, createForgeSource, forgeItemLabel, renderForgeReferenceText, FORGE_SOURCE, forgeItems, forgeAttached };
