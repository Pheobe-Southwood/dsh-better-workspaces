...
`;
		...
		const zh = {
			...
			"hero.stageHint": "发送首条消息时自动创建",
			"hero.stageCreateFallback": "立即创建",
			"hero.stageAttachWarn": "草稿含附件：请先点「立即创建」，再在新会话发送",
		...
		const en = {
			...
			"hero.stageHint": "created when you send the first message",
			"hero.stageCreateFallback": "Create now",
			"hero.stageAttachWarn": "Draft has attachments: use Create now first, then send in the new session",
		...
		/* the client list snapshot lags the create round-trip (push-based sync);
		 * connectWorkspace reads it synchronously and throws `unknown workspace`
		 * otherwise — poll until the new workspace lands (3 s budget) */
		async function waitForWorkspaceInSnapshot(workspaceId, budgetMs) {
			const deadline = Date.now() + budgetMs;
			for (;;) {
				const hit = workspaceItems().find((w) => w.workspaceId === workspaceId);
				if (hit) return hit;
		...
		async function openWorkspaceFor(path) {
			...
			if (!workspace) {
				const created = await appCtx.workspaces.create({ path });
				if (!created || !created.ok) {
					throw new Error(failureMessage(created) || "workspace create failed");
				}
				workspace = created.value.workspace;
			}
			await waitForWorkspaceInSnapshot(workspace.workspaceId, 3000);
		...
		/* first-send interception helpers (pure, unit-tested via __bwTest) */

		/** Nearest ancestor (or self) whose subtree holds the composer text projection. */
		function composerBlockOf(target) {
			let el = target;
			let guard = 0;
			while (el && el !== document.body && guard < 40) {
				if (el.querySelector && el.querySelector("[data-composer-text-ref]")) return el;
				el = el.parentElement;
				guard += 1;
			}
			return null;
		...
		/** Draft text of one composer block, or "" when unreadable/empty. */
		function draftTextOf(block) {
			if (!block || !block.querySelector) return "";
			const ref = block.querySelector("[data-composer-text-ref]");
			if (!ref) return "";
			return String(ref.textContent || "").trim();
		}

		/**
		 * Send-button candidate: enabled, svg-bearing, geometrically last
		 * (bottom-right) button of the composer block.
		 */
		function pickSendCandidate(buttons) {
			let best = null;
			...
			for (const btn of buttons) {
				...
				if (!rect) continue;
				const score = rect.bottom * 100000 + rect.right;
				if (score > bestScore) { bestScore = score; best = btn; }
			}
			return best;
		}

		/** Workspace title from the first prompt line (paseo-style prompt-derived). */
		function promptTitle(text) {
			const line = String(text || "").split(/\r?\n/)[0].replace(/\s+/g, " ").trim();
			if (!line) return "worktree";
		...
		function HeroControl() {
			...
			/* staging refs for the document-level first-send intercept */
			const stagingRef = react.useRef(null);
			const busyRef = react.useRef(null);
			stagingRef.current = staging ? { explicitName, branchPick } : null;
			...
			function isBlankSession() {
				try {
					...
					if (!snap || !snap.current) return true;
					const summary = snap.byId ? snap.byId[snap.current] : null;
					return !summary || summary.blank !== false;
				} catch {
					return true;
				}
			}
			...
			function currentSessionId() {
				try {
					const snap = appCtx.sessions.list && appCtx.sessions.list.getSnapshot ? appCtx.sessions.list.getSnapshot() : appCtx.sessions.getSnapshot && appCtx.sessions.getSnapshot();
			...
			/* paseo-aligned flow: create the staged worktree, jump, optionally
			 * deliver the intercepted first message into the new session */
			async function runStagedFlow(text, withSend) {
				setBusy(t("hero.creating"));
				setError(null);
				setAttachWarn(false);
				try {
					const body = { cwd, intent: "branch-off", slug: mnemonicSlug() };
					if (explicitName) body.branchName = explicitName;
					else body.base = branchPick || (branches && branches.defaultBranch) || undefined;
					const result = await apiPost("/worktrees", body);
					if (!result.ok) throw new Error(failureMessage(result) || "worktree create failed");
					setBusy(t("hero.switching"));
					const opened = await openWorkspaceFor(result.path);
					if (opened.workspace && text) {
						const title = promptTitle(text);
						try {
							const workspaces = appCtx.workspaces;
							let renamed = await workspaces.rename({ workspaceId: opened.workspace.workspaceId, title });
							if (!renamed.ok && renamed.error && renamed.error.code === "workspace/name-conflict") {
								renamed = await workspaces.rename({ workspaceId: opened.workspace.workspaceId, title: title + "-2" });
							}
						} catch { /* cosmetic */ }
					}
					if (withSend && text && opened.sessionId) {
						const conversation = appCtx.get("conversation");
						if (conversation && typeof conversation.sendSession === "function") {
							await conversation.sendSession(opened.sessionId, text, []);
						}
						try {
							const old = currentSessionId();
							if (old && old !== opened.sessionId) localStorage.removeItem("dsh.conversation." + old);
						} catch { /* cosmetic */ }
					}
					setStaging(false);
			...
			/* capture-phase first-send intercept (ADR 0004 amendment): while
			 * staging is armed on a blank session, the composer's send (Enter or
			 * the send button) creates the worktree first and delivers the text
			 * into the new session instead of the original one */
			react.useEffect(() => {
				function intercept(e, isKey, btn) {
					if (!stagingRef.current || busyRef.current) return;
					const block = composerBlockOf(e.target);
					if (!block) return;
					if (block.querySelector("[data-composer-chip]")) {
						e.preventDefault();
						e.stopPropagation();
						setAttachWarn(true);
						return;
					}
					const text = draftTextOf(block);
					if (!text) return;
					if (!isKey) {
						const candidate = pickSendCandidate(block.querySelectorAll("button"));
						if (!candidate || btn !== candidate) return;
					}
					if (!isBlankSession()) return;
					e.preventDefault();
					e.stopPropagation();
					runStagedFlow(text, true);
				}
				const onKey = (e) => {
					if (e.key !== "Enter" || e.shiftKey || e.isComposing) return;
					intercept(e, true, null);
				};
				const onClick = (e) => {
					const btn = e.target && e.target.closest ? e.target.closest("button") : null;
					if (!btn) return;
					intercept(e, false, btn);
				};
			...
			return h(react.Fragment, null,
				...
				staging && attachWarn ? h("span", { className: "dsh-bw-hero-error" }, t("hero.stageAttachWarn")) : null,
				staging ? h("button", {
					type: "button", className: "dsh-bw-btn", disabled: Boolean(busy),
					onClick: () => {
						const ref = document.querySelector("[data-composer-text-ref]");
						const block = ref ? composerBlockOf(ref) : null;
						const text = block ? draftTextOf(block) : "";
						runStagedFlow(text || null, Boolean(text));
					},
		...
		exports.__bwTest = { pickSendCandidate, draftTextOf, promptTitle, failureMessage, mnemonicSlug };
