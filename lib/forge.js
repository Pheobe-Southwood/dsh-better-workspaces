...
export async function ghAuthenticated(host = 'github.com') {
  ...
  const r = await execGh(['auth', 'status', '--hostname', key], { timeout: 15000 });
...
export async function prStatusBatch(targets) {
  ...
  for (const target of targets) {
    const host = String(target.host || 'github.com').toLowerCase();
    if (!(await ghAuthenticated(host))) {
      out.set(target.key, { pr: null, forgeAuth: 'unauthenticated' });
      continue;
    }
  ...
  for (const [host, fresh] of freshByHost) {
    for (let i = 0; i < fresh.length; i += 20) {
      ...
      const epoch = prEpoch;
      const r = await execGh(['api', '--hostname', host, 'graphql', '-f', `query=query{${parts.join('')}}`], { timeout: 45000 });
      if (epoch !== prEpoch) {
        const retried = await prStatusBatch(chunk);
...
export async function listForgeItems(opts) {
  ...
  if (!(await ghAvailable())) return { items: [], authState: 'cli_missing', repository: identity };
  if (!(await ghAuthenticated(identity.host))) return { items: [], authState: 'unauthenticated', repository: identity };
  ...
  const [pulls, issues] = await Promise.all([
    ...
    execGh(['issue', 'list', '--repo', ghRepo(identity), '--search', query, '--json', fields, '--limit', String(limit)], { cwd }),
  ]);
  ...
  const partial = pullResult.failed || issueResult.failed;
  const value = {
    items,
    authState: partial ? 'partial' : 'authenticated',
    repository: identity,
    ...(partial ? { error: (pullResult.failed ? pulls.stderr : issues.stderr).trim().slice(0, 400) || 'partial forge listing failure' } : {}),
  };
  setBounded(listCache, key, { at: Date.now(), value });
  return value;
}
...
export async function pullRequestDetail(opts) {
  ...
  if (!(await ghAvailable())) return { ok: false, authState: 'cli_missing', repository: identity, message: 'gh CLI not installed' };
  if (!(await ghAuthenticated(identity.host))) return { ok: false, authState: 'unauthenticated', repository: identity, message: 'gh is not authenticated for this host' };
