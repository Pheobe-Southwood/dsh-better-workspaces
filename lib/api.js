...
export function createApi(hub, options = {}) {
  ...
  const withStableRoot = async (auth, task) => {
    if (process.platform !== 'linux') throw new ApiError(503, 'stable workspace mutations require Linux dirfd support');
    ...
    try {
      ...
      if (identity.dev !== auth.identity?.dev || identity.ino !== auth.identity?.ino) {
        throw new ApiError(409, 'workspace identity changed at mutation commit point');
      }
      if (auth.mainRepoRoot && auth.mainRepoRoot !== auth.cwd) {
        ...
        if (mainIdentity.dev !== auth.mainIdentity?.dev || mainIdentity.ino !== auth.mainIdentity?.ino) {
          throw new ApiError(409, 'repository family identity changed at mutation commit point');
        }
      }
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && sub === '/pulls') {
        ...
        const identity = await resolveForgeIdentity(detect.repoRoot);
        if (!identity) return sendJson(res, 200, { ok: true, isGit: true, items: [], authState: 'no_remote' });
        const result = await listForgeItems({ cwd: detect.repoRoot, identity, query: '', limit });
      ...
      if (req.method === 'GET' && sub === '/pull') {
        ...
        const actualIdentity = await resolveForgeIdentity(detect.repoRoot);
        if (!actualIdentity) return sendJson(res, 422, { ok: false, authState: 'no_remote', message: 'GitHub repository identity unavailable' });
        if (selectorParts.every(Boolean)) {
          ...
          if (forgeRepositoryKey(requestedIdentity) !== forgeRepositoryKey(actualIdentity)) {
            return sendJson(res, 409, { ok: false, error: 'repository identity changed' });
          }
        }
        const result = await pullRequestDetail({ cwd: detect.repoRoot, identity: actualIdentity, number, kind });
        if (!result.ok) {
      ...
      if (req.method === 'POST' && sub === '/worktrees') {
        ...
        if (pull) {
          ...
          const detail = await resolvePull({ cwd: auth.cwd, identity: actualIdentity, number: pull.number, kind: 'change_request' });
          if (!detail?.ok) {
          ...
          return withStableRoot(current, async (stableRoot) => {
            ...
            const actionResult = await executeAction(hub, stableRoot, body.name, params, {
              ...
              beforeArchiveRemove: body.name === 'archive'
                ...
                : null,
              authorizeTarget: (candidate) => authorizer.authorize(candidate),
            });
            if (deferred && deletionJournal) {
