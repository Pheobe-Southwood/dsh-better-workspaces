...
export function createApi(hub, options = {}) {
  ...
  const workspaceIdForPath = async (path) => {
    if (!workspaceRows) return null;
    const rows = await workspaceRows();
    const hit = Array.isArray(rows) ? rows.find((workspace) => workspace?.path === path) : null;
    return hit ? (hit.workspaceId ?? hit.id ?? null) : null;
  };
  const convergeArchivedWorkspace = async (result, workspaceId, journalFile) => {
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && sub === '/worktrees/archive') {
        ...
        if (!auth) return;
        const workspaceId = await workspaceIdForPath(auth.cwd);
        const result = await mutations.run(mutationKeyFor(auth), async () => {
          ...
          return withStableRoot(current, async (stableRoot) => {
            const deletionJournal = workspaceId !== null
              ...
              : null;
            const archived = await archiveWorktree(stableRoot, { force: Boolean(body.force) });
            return convergeArchivedWorkspace(archived, workspaceId, deletionJournal);
          });
        }, { signal: requestAbort.signal });
      ...
      if (req.method === 'POST' && sub === '/action') {
        ...
        const workspaceId = body.name === 'archive' ? await workspaceIdForPath(auth.cwd) : null;
        let result = await mutations.run(mutationKeyFor(auth), async () => {
          ...
          return withStableRoot(current, async (stableRoot) => {
            ...
            return convergeArchivedWorkspace(actionResult, workspaceId, deletionJournal);
          });
        }, { signal: requestAbort.signal });
        try {
          await assertAuthorizationCurrent(auth);
