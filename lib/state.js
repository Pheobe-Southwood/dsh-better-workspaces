...
export function createGitStateHub({ onChange } = {}) {
  ...
  async function computeTarget(target, force = false) {
    ...
    const promise = (async () => {
      ...
      const untrackedCount = status.entries.filter((e) => e.code === '??').length;
  ...
  async function runDueFetches() {
    ...
    for (const [mainRoot, repo] of [...repos.entries()]) {
      ...
      repo.fetching = true;
      runGit(['fetch', 'origin', '--prune'], { cwd: mainRoot, timeout: 120000, allowFail: true })
        .then(() => {
  ...
  return {
    ...
    /** Force an immediate recompute (after mutations / session events). */
    invalidate(cwd) {
      const target = targets.get(cwd);
