    ...
    const promise = (async () => {
      ...
      try {
        ...
        if (typeof authorizeTarget === 'function') {
          ...
          cwd = `/proc/${process.pid}/fd/${directory.fd}`;
        }
        let pinned = null;
        if (directory) {
          const anchoredPath = (targetPath) => {
            ...
            if (mainDirectory) {
              ...
              if (inMain === '' || (inMain !== '..' && !inMain.startsWith(`..${sep}`) && resolve(inMain) !== inMain)) {
                return inMain === '' ? `/proc/${process.pid}/fd/${mainDirectory.fd}` : `/proc/${process.pid}/fd/${mainDirectory.fd}/${inMain}`;
              }
            }
            return targetPath;
          };
          ...
          pinned = {
            GIT_DIR: `/proc/${process.pid}/fd/${gitDirectory.fd}`,
            GIT_COMMON_DIR: `/proc/${process.pid}/fd/${commonDirectory.fd}`,
            GIT_WORK_TREE: `/proc/${process.pid}/fd/${directory.fd}`,
          };
        }
      ...
      const fetchTask = mutations.run(`git:${mainRoot}`, async () => {
        ...
        try {
          ...
          if (authorizedCapability) {
            ...
            stableCwd = `/proc/${process.pid}/fd/${(mainDirectory || directory).fd}`;
            ...
            const anchoredCommon = familyDirectory && (inMain === '' || (inMain !== '..' && !inMain.startsWith(`..${sep}`) && resolve(inMain) !== inMain))
              ? inMain === '' ? `/proc/${process.pid}/fd/${familyDirectory.fd}` : `/proc/${process.pid}/fd/${familyDirectory.fd}/${inMain}`
              : commonPath;
          ...
          return await (commonDirectory
            ? withPinnedGitEnvironment({
                GIT_DIR: `/proc/${process.pid}/fd/${commonDirectory.fd}`,
                GIT_COMMON_DIR: `/proc/${process.pid}/fd/${commonDirectory.fd}`,
                GIT_WORK_TREE: `/proc/${process.pid}/fd/${(mainDirectory || directory).fd}`,
              }, fetchOperation)
            : fetchOperation());
        } finally {
