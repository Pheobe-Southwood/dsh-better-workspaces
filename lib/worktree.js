...
async function writePendingJournal(root, transaction) {
  const name = `${PENDING_PREFIX}${transaction.txId}${PENDING_SUFFIX}`;
...
export async function prepareWorkspaceDeletion(mainRepoRoot, path, workspaceId) {
...
export async function pendingWorkspaceDeletions(mainRepoRoot) {
  ...
  for (const name of (await readdir(root)).filter((entry) => entry.startsWith(REGISTRY_DELETE_PREFIX) && entry.endsWith(PENDING_SUFFIX))) {
    ...
    const rowRemains = rows.some((row) => row.split('\n').includes(`worktree ${record.path}`));
    if (pathGone && !rowRemains) ready.push({ ...record, file });
  }
  return { ready, errors };
}
...
export async function recoverPendingTransactions(mainRepoRoot) {
  ...
  for (const name of names) {
    ...
    const row = beforeList.stdout
      .split(/\n\s*\n/)
      .find((entry) => entry.split('\n').includes(`worktree ${transaction.path}`));
  ...
  for (const entry of listed) {
    try {
      if (!entry.bare && (await realpath(entry.path)) === cwd) {
        ...
        return { ok: true, cwd, root: cwd, mainRepoRoot: metadataMain, metadata, actualBranch: entry.branch };
      }
    } catch {
...
async function fetchPrHead(mainRoot, number, headRef) {
  ...
  try {
    ...
    if (remains.ok || typeof remains.code !== 'number') {
      throw new Error(`worktree: temporary PR ref cleanup failed: ${cleanupError || remains.error || tempRef}`);
    }
  }
}
...
export async function createWorktree(opts) {
  ...
  const recovery = await recoverPendingTransactions(mainRoot);
  ...
  try {
    ...
    const [bindingList, bindingHead] = await Promise.all([
      ...
      runGit(['rev-parse', '--verify', 'HEAD'], { cwd: finalPath }),
    ]);
    const bindingRow = bindingList.ok && bindingList.stdout
      .split(/\n\s*\n/)
      .find((row) => row.split('\n').includes(`worktree ${finalPath}`));
    if (!bindingRow
    ...
    const gitDirResult = await runGit(['rev-parse', '--path-format=absolute', '--git-dir'], { cwd: finalPath });
  ...
  const metadata = {
    ...
    const committed = await validateManagedWorktree(finalPath, { expectedMainRoot: mainRoot });
    ...
    const createdHead = await runGit(['rev-parse', '--verify', 'HEAD'], { cwd: finalPath });
...
export async function archiveWorktree(path, opts = {}) {
  ...
  if (process.platform === 'linux' && !opts.stableDirfd && typeof path === 'string' && !/^\/proc\/self\/fd\/\d+$/.test(path)) {
    let directory;
    try {
    ...
    try {
      return await archiveWorktree(`/proc/self/fd/${directory.fd}`, { ...opts, stableDirfd: true });
    } finally {
  ...
  const removed = await runGit(removeArgs, { cwd: stableSelector, timeout: 60000 });
  if (!removed.ok) {
    return {
      ok: false,
