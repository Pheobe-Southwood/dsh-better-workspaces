...
async function fetchPrHead(mainRoot, number, headRef) {
  ...
  for (const candidate of candidates) {
    // NOT `allowFail`: that option makes runGit report ok:true for EVERY
    // non-zero exit, which left the failure branch below unreachable (a plain
    // "couldn't find remote ref" was reported as "fetched but the ref did not
    // resolve") and let a stale temp ref from an earlier run pass for a fresh
    // fetch. The real stderr is what names the reason.
...
export async function createWorktree(opts) {
  ...
  if (prUpstream && prHeadSha) {
    ...
    // no `allowFail` on either call: it forces ok:true, which would make the
    // two guards below dead code and report `upstream` for a branch that has
    // none (metadata and return value would claim a tracking ref that
    // `@{upstream}` cannot resolve).
